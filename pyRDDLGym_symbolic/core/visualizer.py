...
class RDDL2Graph:
    ...
    def get_graph_of_cpf(self, fluent: str, gfluent: Optional[str] = None) -> Graph:
        ...
        assert not gfluent or (gfluent and f"{fluent}___{gfluent}" in self.model.variable_base_pvars), (
            f'Grounded fluent {gfluent} provided but cannot be resolved\n'
            f'Failed to find {fluent}___{gfluent} in `variable_base_pvars`'
        )
        ...
        if fluent.lower() == 'reward':
            ...
            gvars = {
                gvar for gvar, pvar in self.model.variable_base_pvars.items() 
                    if pvar == fluent and
                        (not gfluent or (gfluent and f"{fluent}___{gfluent}" == gvar))
            }
    ...
    def get_objects(gvar: str, pvar: str) -> List[str]:
        return [v_str for v_str in gvar.split(pvar)[1].split('_')[1:] if v_str]
    ...
    def get_node_name(self, gvar: str, primed: bool=False) -> str:
        ...
        pvar = self.get_unprimed_pvar_from_gvar(gvar)
    ...
    def get_unprimed_pvar_from_gvar(self, gvar: str):
        """Returns the unprimed pvar name from a gvar name."""
